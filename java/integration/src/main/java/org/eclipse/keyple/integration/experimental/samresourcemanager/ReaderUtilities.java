...
public class ReaderUtilities {
    final static String PO_READER_NAME_REGEX = ".*(ASK|ACS|SpringCard).*";
    final static String SAM_READER_NAME_REGEX =
    ...
    public static ReaderType getReaderType(String readerName) {
        Pattern p;
        p = Pattern.compile(PO_READER_NAME_REGEX);
        if (p.matcher(readerName).matches()) {
            return ReaderType.CONTACTLESS_READER;
        } else {
            p = Pattern.compile(SAM_READER_NAME_REGEX);
            if (p.matcher(readerName).matches()) {
