...
public class PoSelectionRequestTest {
    ...
    public void prepareReadRecordsCmd() {
        poSelectionRequest.prepareReadRecordsCmd((byte) 0x01, ReadDataStructure.SINGLE_RECORD_DATA,
                (byte) 0x01, "Read record SFI 01");
        poSelectionRequest.prepareReadRecordsCmd((byte) 0x02, ReadDataStructure.SINGLE_COUNTER,
                (byte) 0x01, "Read counter SFI 02");
        ...
        ApduResponse fciData = new ApduResponse(ByteArrayUtil.fromHex("6F 22 84 08 " + DF_NAME
                + "A5 16 BF0C 13 C7 08 " + SERIAL_NUMBER + "53 07 060A 27 02200311 9000"), null);
        CalypsoPo calypsoPo = poSelectionRequest.parse(new SeResponse(true, false,
                new SelectionStatus(atr, fciData, true), apduResponseList));

        Assert.assertArrayEquals(ByteArrayUtil.fromHex(RECORD_CONTENT),
                calypsoPo.getRecord((byte) 0x01, (byte) 0x01));
        // TODO Add test for counters when available
    ...
    // TODO update this test when ready
    @Test
    public void prepareSelectFileCmd1() {
        poSelectionRequest.prepareSelectFileCmd(SelectFileCmdBuild.SelectControl.CURRENT_DF,
                "CurrentDF");
        poSelectionRequest.prepareSelectFileCmd(SelectFileCmdBuild.SelectControl.FIRST, "First");
        poSelectionRequest.prepareSelectFileCmd(SelectFileCmdBuild.SelectControl.NEXT, "Next");
